...
function onQuery(collection, queries, onResult, onError) {
    try {
        const { firestoreQuery, cursors } = queries.reduce((acc, q) => {
            switch (q.type) {
                case 'order': {
                    ...
                    acc.firestoreQuery = acc.firestoreQuery.orderBy(field.toString(), method);
                    if (cursors)
                ...
                case 'where': {
                    ...
                    acc.firestoreQuery = acc.firestoreQuery.where(fieldName, filter, data_1.unwrapData(value));
                    break;
                }
                case 'limit': {
                    const { number } = q;
        ...
        return paginatedFirestoreQuery.onSnapshot((firestoreSnap) => {
            onResult(firestoreSnap.docs.map(d => doc_1.doc(collection.__type__ === 'collectionGroup'
                ? ref_1.pathToRef(d.ref.path)
                : ref_1.ref(collection, d.id), data_1.wrapData(d.data()))));
        }, onError);
    }
    catch (err) {
